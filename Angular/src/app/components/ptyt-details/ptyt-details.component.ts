...
@Component({
  selector: 'app-ptyt-details',
  standalone: true,
  imports: [CommonModule, FormsModule, NgbModule, NgIcon],
  templateUrl: './ptyt-details.component.html',
  styleUrl: './ptyt-details.component.scss'
})
export class PtYtDetailsComponent implements OnInit {
    selectedSYForMerge = '';
    mergeAmount = '';
  ...
  selectedSYTokenValue = computed(() => this.selectedSYToken());
    ...
    async ngOnInit() {
      ...
      const staked = this.route.snapshot.paramMap.get('staked');
    ...
    async loadStakingResult(underlying: string, maturity: string) {
      try {
        ...
        console.log('syTokenAddress',syTokenAddress)
