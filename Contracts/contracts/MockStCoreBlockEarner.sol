...
/**
 * @title MockStCoreBlockEarner
 * @dev Mock staked CORE token for demo purposes with per-block yield generation
 * @notice Earns 0.001% per block for easy demonstration of yield accumulation
 */
contract MockStCoreBlockEarner is ERC20, Ownable {
    uint256 public constant INITIAL_SUPPLY = 1000000 * 10**18; // 1M tokens
    
    // Block-based yield tracking
    uint256 public yieldRatePerBlock = 1; // 0.001% per block in basis points (0.001% = 1 bp per 100,000)
    ...
    constructor() ERC20("Mock StCORE", "stCORE") Ownable(msg.sender) {
    ...
    function updateYield() public {
        ...
        // 0.001% per block = 1 basis point per 100,000 = totalSupply * 1 * blocksElapsed / 100,000,000
        uint256 yieldToDistribute = totalSupply() * yieldRatePerBlock * blocksElapsed / 100000000;
    ...
    function getExpectedYieldForBlocks(address user, uint256 blocks) external view returns (uint256) {
        ...
        uint256 yieldForBlocks = userBalance * yieldRatePerBlock * blocks / 100000000;
     ...
     * @param _yieldRatePerBlock New yield rate in basis points per 100,000 (1 = 0.001% per block)
     */
